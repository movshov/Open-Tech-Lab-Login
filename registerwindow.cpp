...
void RegisterWindow::showRules(){
    ...
    QTextStream in(&file);
    rulesDisplay = new QPlainTextEdit();

    rulesDisplay->setReadOnly(TRUE);//in.readAll())
    rulesDisplay->insertPlainText(in.readAll());
    rulesDisplay->resize(500,500);
    rulesDisplay->setFont(rulesFont);
    rulesDisplay->show();
}
